...
module.exports = function (io) {
  ...
  function sendBalanceUpdate(portfolioId, balance, compBalance) {
    const message = JSON.stringify({ portfolioId, balance, compBalance });
    clients.forEach(client => {
      client.emit('balanceUpdate', message);
    });
  }
  ...
  const calculateDailyInterval = portfolio => {
    ...
    const dayOfWeek = new Date(portfolio.dateOfPurchase).getDay();
    return totalDays > 0
      ? portfolio.portConfig[portfolio.payout] /
          (totalDays * daysOfTheWeek[dayOfWeek])
      : 0;
  };
